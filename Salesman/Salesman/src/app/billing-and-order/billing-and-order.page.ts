...
@Component({
  selector: 'app-billing-and-order',
  templateUrl: './billing-and-order.page.html',
  styleUrls: ['./billing-and-order.page.scss'],
})
export class BillingAndOrderPage implements OnInit {
  billing: string;
  getComplaints: any;
  getOrders: any;
  Order_Id: any;
...
this.platform.ready().then(()=>{
  ...
  viewOrders(){
    let Data = {}
    this.dataService.postData('http://esmartpos-com.stackstaging.com/orderdcrud/orderg',Data).subscribe(
      (res)=>{
        let data = res.json();
        this.getOrders=data;
        console.log(JSON.stringify(this.getOrders));
      }
    );21
  }

  RedirectMeToOrders(Order_Id){
    console.log(Order_Id);
    this.navCtrl.navigateForward('/billing-and-order2/'+Order_Id);
  }
}
